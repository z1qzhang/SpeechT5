...
class SpeechLM(nn.Module):
    def __init__(
        ...
        if self.add_unit_encoder:
            ...
            unit_dictionary = Dictionary.load("/home/ziqzhang/dataset-diskd/LibriLM/phone_unit/bin-idx/dict.phn.txt")
            unit_dictionary.add_symbol("<mask>")
        ...
        with torch.no_grad():
            ...
            if self.add_unit_encoder and (output_layer is None or output_layer > self.cfg.encoder_layers):
                ...
                if return_all_hiddens:
                    res["layer_results"] += encoder_out['encoder_states'][0:1+output_layer-len(res["layer_results"])]
